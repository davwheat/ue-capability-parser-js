 ...
 * ```
 */
export default function parseNsgLogToObject(text: string): Record<string, any> {
  ...
  lines.forEach((line, i) => {
    ...
    if (line.match(/.* : .*/)) {
      ...
      const nextLine = peekAhead(i);
      const isSameIndent = nextLine ? nextLine.length - nextLine.trimStart().length === newIndent : false;

      if (nextLine) {
        const isArr = nextLine?.match(ArrayRegexPattern) ? true : false;

        if (!isSameIndent) {
          currentData[currentPath[currentPath.length - 1]][parseInt(index)] ||= isArr ? [] : {};
        } else if (isArr) {
          currentData[currentPath[currentPath.length - 1]][parseInt(index)] ||= null;
        }
      } else {
        // if this is arr index, and end of file, add null
        currentData[currentPath[currentPath.length - 1]][parseInt(index)] ||= null;
      }

      !isSameIndent && currentPath.push(parseInt(index));
    } else {
      // new key
